...
Pythonプログラムはpg27.pyです．このプログラムが読み込む画像ファイルがフォルダimageに，音声ファイルがフォルダ
soundにあります．
...
### プログラムの実行
pg27.pyというPythonプログラムを実行するとゲームが始まります．音が発生しますので初めてのときはご注意ください．
...
## チームでの開発
学生はみんなPython初心者でしたので，前半6週では全員が同じ課題（Pythonの基礎からゲームの基本構造まで）に取り組みました．
後半6週では役割分担（制御，障害物，ゲームパッド，画像，サウンド）を決めて分担作業し，最後に全体を結合して一つのプログラムにしました．

## プログラムの要点
スクリーン上にさまざなな画像を表示させ，あるフレームレートでそれらの画像の位置と角度を更新することでアニメーションが表示されます．
この画像表示にはpygame提供のscreen.blitを使うのですが，pygameデフォルトの座標軸とロボット工学の座標軸が異なるため，
この違いを解消するためのdisp_imgという関数を自作しました．
これによりロボット工学で用いる位置座標と姿勢角を使って画像表示のプログラミングが可能になっています．
